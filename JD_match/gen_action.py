...
def draw(actions):
    ...
    action_mar=action_mar.groupby('time').first().reset_index()
    action_mar=action_mar.groupby(['user_id','sku_id']).count().reset_index()
    pic=action_mar.plot(x='time').get_figure()
    pic.savefig('./action_mar.png')
